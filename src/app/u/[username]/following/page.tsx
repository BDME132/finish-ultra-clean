...
async function loadFollowing(userId: string): Promise<FollowProfile[]> {
  const supabase = await createSupabaseServer();
  ...
  const ids = ((rows as { followed_user_id: string }[]) ?? []).map((r) => r.followed_user_id);
...
export async function generateMetadata(
  ...
  return pageMetadata({
    title: `Following of @${username} | FinishUltra`,
    description: `Runners @${username} is following on FinishUltra.`,
    path: `/u/${username}/following`,
  });
}

export default async function PublicFollowingPage({
  params,
}: {
  ...
  if (!profile) notFound();
  const following = await loadFollowing(profile.id);
    ...
    <>
      ...
      <main className="min-h-[calc(100vh-4rem)] bg-light">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <h1 className="font-headline text-3xl font-bold text-dark mt-2 mb-6">Following</h1>
          {following.length === 0 ? (
