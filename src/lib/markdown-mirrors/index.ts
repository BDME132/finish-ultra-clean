...
const blogIndexBase =
  fm(
    ...
    "AI guides and community posts covering training, gear, nutrition, and race day prep for beginner ultra runners.",
    "/blog",
    "2026-04-11"
  ) +
  `# Blog

One blog feed with clearly labeled Pheidi AI guides and community posts from FinishUltra runners.

## AI Disclosure

Some articles are written by Pheidi (AI). These posts are labeled clearly and should not replace professional medical, coaching, or safety advice.
...
async function getBlogPostMarkdown(slug: string): Promise<string | null> {
  ...
  md += `# ${post.title}\n\n`;
  md += `*${getBlogSourceLabel(post.authorType)} · ${post.category} · ${post.readTime} · By ${post.authorName}*\n\n`;
  if (post.authorType === "ai") {
    md += `> Written by Pheidi (AI). This article was generated by AI after reviewing extensive ultrarunning information. Verify important medical, training, and safety decisions independently.\n\n`;
  }
  md += `${post.excerpt}\n\n---\n\n`;
  md += `${post.bodyMarkdown}\n\n`;
  ...
  const relatedPosts = getRelatedBlogPosts(
    post,
    await loadPublicBlogPostsServer(),
    3,
  );
  if (relatedPosts.length > 0) {
    md += `## Related Articles\n\n`;
    for (const related of relatedPosts) {
      md += `- [${related.title}](/blog/${related.slug})\n`;
    }
    md += "\n";
  }
...
export async function getMarkdownForPath(path: string): Promise<string | null> {
  ...
  if (normalized === "/blog") {
    ...
    for (const post of await loadPublicBlogPostsServer()) {
      md += `- [${post.title}](/blog/${post.slug}) — ${getBlogSourceLabel(post.authorType)} · ${post.category} · ${post.readTime}\n`;
    }
    return md;
  }
